...
namespace UFPS.Base.Scripts.Demo
{
    public class vp_DemoManager
    {
        ...
        public bool ButtonToggle(Rect rect, string label, bool state, bool arrow, Texture imageUpPointer)
        {
            ...
            // draw toggle label
            UnityEngine.GUI.Label(new Rect(rect.x, rect.y - 30, rect.width, rect.height), label, CenterStyle);
		
            // draw buttons
            if (UnityEngine.GUI.Button(new Rect(rect.x, rect.y, (rect.width * 0.5f)-2, rect.height), "OFF", offStyle))
                state = false;
            if (UnityEngine.GUI.Button(new Rect(rect.x + (rect.width * 0.5f) + 2, rect.y,
                                    (rect.width * 0.5f), rect.height), "ON", onStyle))
                state = true;

            // if button has an arrow, draw it
            if(arrow)
                UnityEngine.GUI.Label(new Rect(rect.x + ((rect.width * 0.5f) * 0.5f) - 14 + arrowOffset,
        ...
        public void DrawBoxes(string caption, string description, Texture imageLeftArrow, Texture imageRightArrow, LoadLevelCallback nextLevelCallback = null, LoadLevelCallback prevLevelCallback = null, bool drawBox = true)
        {
            ...
            UnityEngine.GUI.color = new Color(1, 1, 1, 1 * GlobalAlpha);
            ...
            if (imageLeftArrow != null)
                UnityEngine.GUI.Box(new Rect(30, 10, 80, 80), "");

            // draw big text background box
            if(drawBox)
                UnityEngine.GUI.Box(new Rect(120, 0, 560, 100), "");

            UnityEngine.GUI.color = new Color(1, 1, 1, m_TextAlpha * GlobalAlpha);
            ...
            if (imageRightArrow != null)
                UnityEngine.GUI.Box(new Rect(690, 10, 80, 80), "");

            // draw 'prev' arrow button, if texture is provided
            if (imageLeftArrow != null)
            {
                if (UnityEngine.GUI.Button(new Rect(35, 15, 80, 80), imageLeftArrow, "Label"))
                {
            ...
            UnityEngine.GUI.color = new Color(1, 1, 1, m_BigArrowFadeAlpha * GlobalAlpha);

            // draw 'next' arrow button, if texture is provided, unless
            // a checkmark texture is provided ...
            if (imageRightArrow != null)
            {
                if (UnityEngine.GUI.Button(new Rect(700, 15, 80, 80), imageRightArrow, "Label"))
                {
        ...
        public int ToggleColumn(int width, int y, int sel, string[] strings, bool center, bool arrow, Texture imageRightPointer, Texture imageLeftPointer)
        {
            ...
            foreach (string s in strings)
            {
                ...
                if (v == sel)
                {
                    ...
                    {
                        if (center)
                            UnityEngine.GUI.Label(new Rect(rect.x - 27, rect.y, 32, 32), imageRightPointer);
                        else
                            UnityEngine.GUI.Label(new Rect(rect.x + rect.width + 5, rect.y, 32, 32), imageLeftPointer);
                    }
                ...
                if (UnityEngine.GUI.Button(rect, s, style))
                    sel = v;
        ...
        public int ButtonColumn(int y, int sel, string[] strings, Texture imagePointer)
        {
            ...
            foreach (string s in strings)
            {
                ...
                if (UnityEngine.GUI.Button(rect, s))
                {
                    sel = v;
            ...
            // draw the arrow
            UnityEngine.GUI.color = new Color(1, 1, 1, Mathf.Max(0, 1 - (Time.time - ButtonColumnClickTime) * 1.0f * GlobalAlpha));
            UnityEngine.GUI.Label(new Rect(rect.x - 27, ButtonColumnArrowY, 32, 32), imagePointer);
            UnityEngine.GUI.color = new Color(1, 1, 1, 1 * GlobalAlpha);
        ...
        public void DrawImage(Texture image, float xOffset, float yOffset)
        {
            ...
            UnityEngine.GUI.DrawTexture(new Rect(screenCenter - (width / 2) + xOffset, 140 + yOffset, width, width * aspect), image);
        ...
        public void DrawEditorPreview(Texture section, Texture imageEditorPreview, Texture imageEditorScreenshot)
        {
            ...
            if (EditorPreviewSectionExpanded)
            {
                ...
                UnityEngine.GUI.DrawTexture(new Rect(xPos, captionY, caption.width, caption.height), caption);
                UnityEngine.GUI.DrawTexture(new Rect(xPos, sectionY, section.width, section.height), section);
                ...
                UnityEngine.GUI.color = new Color(1, 1, 1, (col.a * 0.5f) * m_EditorPreviewScreenshotTextAlpha * GlobalAlpha);
                UnityEngine.GUI.DrawTexture(new Rect(xPos + 48, sectionY + (section.height / 2) - (imageEditorScreenshot.height / 2),
        ...
        public void DrawFullScreenText(Texture imageFullScreen)
        {
            ...
            UnityEngine.GUI.color = new Color(1, 1, 1, m_FullScreenTextAlpha * GlobalAlpha);
            UnityEngine.GUI.DrawTexture(new Rect((Screen.width / 2) - 120, (Screen.height / 2) - 16, 240, 32), imageFullScreen);
            UnityEngine.GUI.color = new Color(1, 1, 1, 1 * GlobalAlpha);
        ...
        public void OnGUI()
        {
            ...
            if ((Time.time - CurrentFullScreenFadeTime) < m_FullScreenFadeInDuration)
            {
                ...
                UnityEngine.GUI.color = new Color(0, 0, 0, (m_FullScreenFadeInDuration - GlobalAlpha) / m_FullScreenFadeInDuration);
                UnityEngine.GUI.DrawTexture(new Rect(0, 0, Screen.width, Screen.height), m_FullScreenFadeTexture);
            ...
            if (CurrentFullScreenFadeTime > Time.time)
            {
                ...
                UnityEngine.GUI.color = new Color(0, 0, 0, (m_FullScreenFadeOutDuration - GlobalAlpha) / m_FullScreenFadeOutDuration);
