...
namespace UFPS.Multiplayer.Scripts.Demo.GUI
{
    ...
    public class vp_MPDemoChat : MonoBehaviour
    {
        ...
        protected virtual void OnGUI()
        {
            ...
            {
                UnityEngine.GUI.FocusControl("TextInput");
            ...
            UnityEngine.GUI.BeginScrollView(m_ViewRect, m_ScrollPosition, new Rect(0, 0, m_ViewRect.width - m_LineHeight, LogPixelHeight));
            for (int v = 0; v < m_Messages.Count; v++)
            {
                UnityEngine.GUI.color = Color.white;
                UnityEngine.GUI.Label(new Rect(0, v * m_LineHeight, Screen.width * 0.5f, m_LineHeight + 8), m_Messages[v], TextStyle);
            }
            UnityEngine.GUI.EndScrollView();

            // draw our own scrollbar on the left side
            if ((ViewRectHeight < LogPixelHeight))
                m_ScrollPosition.y = UnityEngine.GUI.VerticalScrollbar(m_ScrollbarRect, m_ScrollPosition.y, (Screen.height * 0.33f), 0, LogPixelHeight);
            ...
            if (m_TextInputVisible)
            {
                UnityEngine.GUI.SetNextControlName("TextInput");
                m_InputLine = UnityEngine.GUI.TextField(m_TextFieldRect, m_InputLine, MaxMessageLength);
