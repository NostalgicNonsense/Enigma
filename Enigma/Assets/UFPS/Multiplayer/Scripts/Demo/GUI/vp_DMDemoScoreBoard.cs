...
using UFPS.Multiplayer.Scripts.Demo.Deathmatch;
...
{
    public class vp_DMDemoScoreBoard : MonoBehaviour
    {
        ...
        void DrawLabel(string text, Vector2 position, Vector2 scale, GUIStyle textStyle, Color textColor, Color bgColor, bool dropShadow = false)
        {
            ...
            if (bgColor != Color.clear)
            {
                ...
                UnityEngine.GUI.DrawTexture(labelRect, Background);
            }


            if (dropShadow)
            {
                UnityEngine.GUI.color = Color.Lerp(bgColor, Color.black, 0.5f);
                shadowRect = labelRect;
                shadowRect.x += scale.y * 0.1f;
                shadowRect.y += scale.y * 0.1f;
                UnityEngine.GUI.Label(shadowRect, text, textStyle);
            }

            UnityEngine.GUI.color = textColor;
            UnityEngine.GUI.Label(labelRect, text, textStyle);
